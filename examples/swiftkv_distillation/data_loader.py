...
def filter_dataset_length(dataset):
    return dataset.filter(
        lambda x: len(x["input_ids"]) <= 8192,
        num_proc=16,
    )
...
def get_dataset(name, tokenizer):
    dataset = load_dataset(name, split="train_sft", num_proc=16)
    dataset = process(dataset, tokenizer)
    dataset = filter_dataset_length(dataset)
    dataset = pack_dataset(dataset)
    return create_dataloader(dataset, tokenizer)
