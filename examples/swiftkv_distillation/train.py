...
if __name__ == "__main__":
    ...
    optimizer = optim.AdamW([p for n, p in smodel.named_parameters() if p.requires_grad],
                            lr=0.0002,
                            betas=[0.9, 0.999])
    ############
    # SCHEDULER
    ...
    def distillation_loss(
        student_output, teacher_output, temperature=1.0, dim=-1
    ):
    ...
    model.to("qaic")
    for epoch_idx in tqdm(num_epochs, total=num_epochs, desc="Training epoch SwiftKV AI100"):
